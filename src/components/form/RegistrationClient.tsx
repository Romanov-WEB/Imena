...
import { phoneExp, emailExp } from '../../lib/RegExp';
...
export default function RegistrationClient():JSX.Element {
    ...
    const onSubmitForm = (e: React.MouseEvent<HTMLButtonElement, MouseEvent>) => {
        ...
        validInput.password = refPassword.current?.value === password && Boolean(password.trim()) && password.length > 6;
    ...
    return (
        <form ref={refForm} id={'registration-client'} className={css.form}>
            <input name={'name'} type={"text"} className={validInput.name? css.input: css.noValidInput} placeholder={'Имя *'}/>
            <input name={'family'} type={"text"} className={validInput.family? css.input: css.noValidInput} placeholder={'Фамилия *'}/>
            <input name={'phone'} type={"number"} className={validInput.phone? css.input: css.noValidInput} placeholder={'Телефон *'}/>
            <input name={'email'} type={"email"} className={validInput.email? css.input: css.noValidInput} placeholder={'Email'}/>
            <input ref={refPassword} name={'password'} type={'password'} className={validInput.password? css.input: css.noValidInput} placeholder={'Пароль *'}/>
            <input name={'password'} type={'password'} className={validInput.password? css.input: css.noValidInput} placeholder={'Пароль еще раз *'}/>
            ...
            <button onClick={(e) => onSubmitForm(e)} className={css.btn}>ВОЙТИ</button>
        </form>
    );
}
